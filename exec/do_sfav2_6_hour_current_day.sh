...
# Script to perform 6-hour National Snowfall Analysis, version 2, with
# projection, GRIB generation, and GISRS layer conversion included.
#
...
# Changes:
#
#   2019-09-16, GF: Moved sourcing of /etc/bashrc above function
#                   definitions to ensure pathmunge and ldpathmunge
#                   functions are not unset.
#   2019-09-20, GF: Added a fancier usage message (including the
#                   "wrecho_hang" function) and the -e option to use
#                   the extended color ramp.
#   2019-09-30, GF: Added the -m option for email recipient/s.
...
usage() {
    echo ""
    echo -n "Usage: do_sfav2_6_hour_current_day.sh "
    HANG_COLS=38
    wrecho_hang "Usage: do_sfav2_6_hour_current_day.sh" \
                "[-h,-?] [-q] [-f] [-l] [-u] [-e] [-m EMAIL]" \
                "[analysis YYYYMMDDHH]"
    echo ""
    echo -n "Options: -h  "
    ...
    wrecho_hang "Including the [analysis YYYYMMDDHH] argument" \
                "performs an analysis for the 6-hour period" \
                "ending at the indicated date/time, rather than" \
                "the default, which is to generate 6-hour" \
                "analyses for all reasonable 00Z and 12Z crossings" \
                "during (roughly) the last 48-hour period."
    echo ""
}
...
while getopts "h?qfluem:" OPT ; do
    case $OPT in
        ...
        l)
            ...
            ;;
        u)
            BUSY_FILE=SNOWFALL_V2_6HR_CURRENT_$(echo $USER | tr [a-z] [A-Z])_BUSY
            ;;
        e)
            EXTENDED_COLORS=1
            ;;
...
if [ -z "$BUSY_FILE" ] ; then
    BUSY_FILE=SNOWFALL_V2_6HR_CURRENT_BUSY
fi
...
#   2. Daily min/max temperatures from SNODAS (keep forever) and
#      HRRR (keep for 30 days):
#      SNODAS_min_max_ave_2m_temp_anl_24h_ending_[0-9]{10}.sav
#      HRRR_min_max_ave_2m_temp_f00_24h_ending_[0-9]{10}.sav
#   3. HRRR and RAP precipitation accumulations (keep for 5 days):
