...
# conversion included.
#
# This version is set up to run analysis for several recent days, but
# not for the current day (see LAG_MINUTES and BASE_TIME).
#
...
#   2019-09-19, GF: Addition of command line options, changes to more
#                   closely match code with do_sfav2_current_day.sh,
#                   and a fancier usage message (including the
...
usage() {
    echo ""
    echo -n "Usage: do_sfav2_multi_day.sh "
    HANG_COLS=29
    ...
    wrecho_hang "Including the [analysis YYYYMMDDHH] argument" \
                ...
                "during (roughly) the four days prior to the" \
                "current date."
    echo ""
}
...
if [ -z "$BUSY_FILE" ] ; then
    BUSY_FILE=SNOWFALL_V2_MULTI_BUSY
fi
...
MAX_RUNTIME_MINUTES_DAILY=90
LOOKBACK_MINUTES_DAILY=$((24*60))
LAG_MINUTES=$((MAX_RUNTIME_MINUTES_DAILY+LOOKBACK_MINUTES_DAILY))
BASE_TIME=`date -u --date="-${LAG_MINUTES} minutes ${DATE}" \
           "+%Y-%m-%d %H:%M:%S"`

MIN_HOURS_BACK=0  # Relative to LAG_MINUTES
MAX_HOURS_BACK=96 # Relative to LAG_MINUTES
if [ $# -eq 1 ] ; then
  MAX_HOURS_BACK=1
fi
