PRO GET_HOURLY_RTMA197_2M_TEMP, $
...
; temperatures. The "197" refers to the 5 km CONUS grid for RTMA and
...
; {RTMADir}/YYYY/MM/DD and files follow the naming convention
; LTMA98_KWBR_{YYYYMMDDHH}00. This is how the data are stored
; (with physical elements in separate files) in the NCEI archive at
...
;+
; Read RTMA (5 km "197" grid) data from GRIB files.
;-
...
;-
  ...
  GRIBFile = 'LTMA98_KWBR_' + hourStartDate_YYYYMMDDHH + '00'

  if NOT(FILE_TEST(GRIBDir + '/' + GRIBFile)) then begin
      if KEYWORD_SET(verbose) then $
          USR_MSG, 'File ' + GRIBDir + '/' + GRIBFile + ' not found.'
      RETURN
  endif
...
;-
  ...
  if NOT(KEYWORD_SET(RTMA197GridProjInfo)) then begin

;+
;     Create the RTMA197GridProjInfo structure to store grid and
...
;-
      ...
      if (nCols ne RTMA197GridProjInfo.nCols) then begin
          ERR_MSG, 'Grid columns in decoded GRIB2 data ' + $
                   '(' + STRCRA(nCols) + ') ' + $
                   'do not match RTMA197_GRID_PROJ_INFO value ' + $
                   '(' + STRCRA(RTMA197GridProjInfo.nCols) + ').'
          dimsOK = 0
      endif

      if (nRows ne RTMA197GridProjInfo.nRows) then begin
          ERR_MSG, 'Grid rows in decoded GRIB2 data ' + $
                   '(' + STRCRA(nRows) + ') ' + $
                   'do not match RTMA197_GRID_PROJ_INFO value ' + $
                   '(' + STRCRA(RTMA197GridProjInfo.nRows) + ').'
          dimsOK = 0
      endif
