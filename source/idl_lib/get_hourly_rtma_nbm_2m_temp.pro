PRO GET_HOURLY_RTMA_NBM_2M_TEMP, $
...
; temperatures. The "NBM" refers to the 2.5 km CONUS grid for RTMA and
; the National Blend of Models (NBM), which replaced grid 184 on
; 2018-12-04 12Z. See
...
;    This is how the data were stored (with all physical elements in
;    one file) in operational data at
;    ftp://ftp.ncep.noaa.gov/pub/data/nccf/com/rtma/prod
;    until 2018-12-04 11Z.
; 3. rtma2p5.{YYYYMMDD}.t{HH}z.2dvaranl_ndfd.grb2_wexp
;    This is how the data were stored (with all physical elements in
;    one file) in operational data at
;    ftp://ftp.ncep.noaa.gov/pub/data/nccf/com/rtma/prod
;    beginning 2018-12-04 12Z.
;
; :Params:
;
...
;+
; Read RTMA (2.5 km "wexp" or "NBM" grid) data from GRIB files.
;-
...
; rtma2p5.{YYYYMMDD}.t{HH}z.2dvaranl_ndfd.grb2_wexp
; This is how the data are stored (with all physical elements in one
; file) in operational data at
; ftp://ftp.ncep.noaa.gov/pub/data/nccf/com/rtma/prod
; since early December 2018.

  GRIBFile = 'rtma2p5.' + hourStartDate_YYYYMMDD + $
             '.t' + hourStartDate_HH + 'z.2dvaranl_ndfd.grb2_wexp'
...
;-
  ...
  if NOT(KEYWORD_SET(RTMA_NBM_GridProjInfo)) then begin

;+
;     Create the RTMA_NBM_GridProjInfo structure to store grid and
...
;+
;     Create data structure for the RTMA "wexp" or "NBM" grid and
;     projection.
;-
      RTMA_NBM_GridProjInfo = $
...
;-
      ...
      if (nCols ne RTMA_NBM_GridProjInfo.nCols) then begin
          ERR_MSG, 'Grid columns in decoded GRIB2 data ' + $
                   '(' + STRCRA(nCols) + ') ' + $
                   'do not match RTMA_NBM_GRID_PROJ_INFO value ' + $
                   '(' + STRCRA(RTMA_NBM_GridProjInfo.nCols) + ').'
          dimsOK = 0
      endif

      if (nRows ne RTMA_NBM_GridProjInfo.nRows) then begin
          ERR_MSG, 'Grid rows in decoded GRIB2 data ' + $
                   '(' + STRCRA(nRows) + ') ' + $
                   'do not match RTMA_NBM_GRID_PROJ_INFO value ' + $
                   '(' + STRCRA(RTMA_NBM_GridProjInfo.nRows) + ').'
          dimsOK = 0
      endif
