PRO GET_MIN_MAX_AVE_RUC130_2M_TEMP, $
...
; from GRIB files, and determine minimum, maximum, and mean values on
; the grid. The "130" refers to the 13 km CONUS grid for RUC. See
...
;-
  ...
  savFile = 'RUC_130_MIN_MAX_AVE_TMP' + $
...
;-
  ...
  for hc = 0, DurationHours - 1 do begin
      ...
      if NOT(ISA(hourlyTmpGrid)) then begin
          ERR_MSG, 'Failed to get RUC ":TMP:2 m above ground:" for hour ' + $
                   hourStartDate_YYYYMMDDHH
...
;-
  ind = WHERE(aveTmpGrid_ eq NoDataValue, count)
  aveTmpGrid_ = aveTmpGrid_ / DurationHours
  if (count gt 0) then aveTmpGrid_[ind] = NoDataValue
