...
;+
; Get accumulated hourly High Resolution Rapid Refresh (HRRR) total
; precipitation (APCP) and water equivalent of accumulated snow depth
; (WEASD) from GRIB files, and regrid/deproject the data to
...
;-
  ...
  savFile = 'HRRR_LONLAT_APCP_WEASD' + $
...
;+
; Read HRRR data from GRIB file/s and regrid. This is the beginning of
; the process if we do not have a SAVE file.
;-
  accumAPCPGridLC = !NULL
  accumWEASDGridLC = !NULL

  GET_ACCUM_HRRR_LC_APCP_WEASD, AccumEndDate_YYYYMMDDHH, $
                                DurationHours, $
                                TargetFcstHour, $
                                MinSubFcstHour, $
                                MaxSubFcstHour, $
  ...
  if (NOT(ISA(accumAPCPGridLC)) or $
      NOT(ISA(accumWEASDGridLC))) then begin
      if KEYWORD_SET(Verbose) then $
          ERR_MSG, 'Failed to get HRRR precipitation for ' + $
...
; Perform regridding.
;-
  accumAPCPGrid_ = REGRID_BILIN(accumAPCPGridLC, $
                                iGrid_HRRR, $
                                jGrid_HRRR, $
                                NoDataValue)
  if NOT(ISA(accumAPCPGrid_)) then RETURN
  accumWEASDGrid_ = REGRID_BILIN(accumWEASDGridLC, $
                                 iGrid_HRRR, $
                                 jGrid_HRRR, $
                                 NoDataValue)
