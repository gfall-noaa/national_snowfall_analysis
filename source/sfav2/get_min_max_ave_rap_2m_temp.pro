PRO GET_MIN_MAX_AVE_RAP_2M_TEMP, $
...
; GRIB files, determine gridded minimum, maximum, and mean values on
; the grid, and regrid/deproject the result to longitude/latitude
; coordinates with the designated geographic domain and resolution.
;
...
;         The directory where Rapid Refresh (RAP) data are stored.
...
;-
  ...
  savFile = 'RAP_LONLAT_MIN_MAX_AVE_TMP' + $
...
;-
  ...
  GET_MIN_MAX_AVE_RAP130_2M_TEMP, $
      AnalysisEndDate_YYYYMMDDHH, $
      DurationHours, $
      TargetFcstHour, $
      MinSubFcstHour, $
  ...
  if (NOT(ISA(minTmpGrid130)) or $
      ...
      ERR_MSG, 'Failed to get min/max/ave RAP temperatures for ' + $
