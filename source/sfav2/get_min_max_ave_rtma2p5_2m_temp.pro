PRO GET_MIN_MAX_AVE_RTMA2P5_2M_TEMP, $
...
; temperatures (TMP) from grid 184 or "wexp/NBM" (the 2.5 km CONUS
; grids) GRIB files, determine gridded minimum, maximum, and mean
; values on the grid, and regrid/deproject the data to
; longitude/latitude coordinates with the designated geographic domain
; and resolution. The "2P5" in the name of this procedure refers not
; to the resolution of the output data, but rather (roughly) to the
; native resolution of the input data used, to distinguish it from 5
; km RTMA data that was the only RTMA CONUS grid available until late
; September 2010. A separate set of procedures must be used to read
; those.
;
; :Params:
;
...
;         the dimensions of grid 184, defined by this structure, will
...
;         A named variable that will capture the structure describing
;         the Lambert conformal 2.5 km "NBM" CONUS grid and coordinate
;         system. If this variable is undefined, it will be
;         defined. If it is already defined, it will be verified
;         against the contents of all the input GRIB2 files. Note that
;         the dimensions of the NBM grid, defined by this structure,
;         will not match those of the output grids, which are on a
;         geographic (longitude/latitude) grid. Structure tags are the
;         same as for RTMA184_GRID_PROJ_INFO, but with different
...
; :History:
;
;     2018-12-07, GF - Added support for "wexp/NBM" grid for RTMA,
;                      which replaced grid 184 on 2018-12-04 12Z.
;-
...
;-
  ...
  savFile = 'RTMA2P5_LONLAT_MIN_MAX_AVE_TMP' + $
...
;+
; Read RTMA data from grid 184 or the "wexp/NBM" grid, from GRIB
; file/s, and regrid. This is really the beginning of the process if
; we do not have a SAVE file.
;-
  minTmpGridProj = !NULL
  maxTmpGridProj = !NULL
  aveTmpGridProj = !NULL
...
;+
; First try the "wexp/NBM" grid.
;-
  GET_MIN_MAX_AVE_RTMA_NBM_2M_TEMP, $ 
      AnalysisEndDate_YYYYMMDDHH, $
      DurationHours, $
      MaxMissingHours, $
      RTMADir, $
  ...
  if (NOT(ISA(minTmpGridProj)) or $
      NOT(ISA(maxTmpGridProj)) or $
      NOT(ISA(aveTmpGridProj))) then begin
...
;-
      ...
      if (NOT(ISA(minTmpGridProj)) or $
          NOT(ISA(maxTmpGridProj)) or $
          NOT(ISA(aveTmpGridProj))) then begin
          ERR_MSG, 'Failed to get min/max/ave RTMA temperatures ' + $
                   'for ' + STRCRA(DurationHours) + ' hours ending ' + $
                   AnalysisEndDate_YYYYMMDDHH
          RETURN
      endif

      RTMAGridProjInfo = RTMA184GridProjInfo

  endif else RTMAGridProjInfo = RTMA_NBM_GridProjInfo
...
;-
  ...
  LCC_GRIB2_TO_SNYDER, RTMAGridProjInfo.latSec1, $
                       RTMAGridProjInfo.latSec2, $
                       RTMAGridProjInfo.latD, $
                       RTMAGridProjInfo.lonV, $
                       RTMAGridProjInfo.lat00, $
                       RTMAGridProjInfo.lon00, $
                       RTMAGridProjInfo.eRadM, $
                       lonV_rad, $
                       nSny, $
                       FSny, $
                       rho0, $
  ...
  degToRad = !DPi / 180.0D
  rho = RTMAGridProjInfo.eRadM * FSny / $
        (TAN(!DPi / 4.0D + latGrid * degToRad / 2.0D))^nSny ; Snyder 15-1
  theta_rad = nSny * (lonGrid * degToRad - lonV_rad)
  x = rho * SIN(theta_rad)        ; x location on RTMA grid
  y = rho0 - rho * COS(theta_rad) ; y location on RTMA grid

  rho = !NULL
  theta_rad = !NULL

  iGrid_RTMA = (x - x00) / RTMAGridProjInfo.dx
  jGrid_RTMA = (y - y00) / RTMAGridProjInfo.dy
...
; Perform regridding.
;-
  minTmpGrid_ = REGRID_BILIN(minTmpGridProj, $
                             iGrid_RTMA, $
                             jGrid_RTMA, $
                             NoDataValue)
  if NOT(ISA(minTmpGrid_)) then RETURN
  maxTmpGrid_ = REGRID_BILIN(maxTmpGridProj, $
                             iGrid_RTMA, $
                             jGrid_RTMA, $
                             NoDataValue)
  if NOT(ISA(maxTmpGrid_)) then RETURN
  aveTmpGrid_ = REGRID_BILIN(aveTmpGridProj, $
                             iGrid_RTMA, $
                             jGrid_RTMA, $
                             NoDataValue)
