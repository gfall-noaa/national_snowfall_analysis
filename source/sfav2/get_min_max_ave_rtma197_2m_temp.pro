PRO GET_MIN_MAX_AVE_RTMA197_2M_TEMP, $
    ...
    RTMA197_GRID_PROJ_INFO = $  ; RTMA 197 grid/projection structure
      RTMA197GridProjInfo, $
    NUM_MISSING_HOURS = $
      numMissingHours, $
...
; and mean values on the grid. The "197" refers to the 5 km CONUS
; grid for RTMA and other NDFD products. See
; `http://www.nco.ncep.noaa.gov/pmb/docs/on388/tableb.html#GRID197
; <http://www.nco.ncep.noaa.gov/pmb/docs/on388/tableb.html#GRID197>`
;
; :Params:
;
...
; :Keywords:
;
;     RTMA197_GRID_PROJ_INFO : type=STRUCT
;         A named variable that will capture the structure describing
;         the Lambert conformal 5 km CONUS grid (#197) and coordinate
...
;-
  ...
  if KEYWORD_SET(RTMA197GridProjInfo) then begin

;+
;     Verify the RTMA197GridProjInfo structure.
;-
      sizeRTMA197GridProjInfo = SIZE(RTMA197GridProjInfo)
      if (sizeRTMA197GridProjInfo[0] ne 1) then begin
          ERR_MSG, 'RTMA197_GRID_PROJ_INFO structure mismatch (non-scalar).'
          RETURN
      endif

      if ((sizeRTMA197GridProjInfo[1] ne 1) or $
          (sizeRTMA197GridProjInfo[2] ne 8)) then begin
          ERR_MSG, 'RTMA197_GRID_PROJ_INFO structure mismatch ' + $
                   '(not a structure).'
          RETURN
      endif

      sizeRTMA197GridProjInfo = !NULL

      structOK = 1

      tagNames = TAG_NAMES(RTMA197GridProjInfo)
      ind = WHERE(tagNames eq 'LONV', count)
      if (count eq 0) then begin
          ERR_MSG, 'No "lonV" tag in RTMA197_GRID_PROJ_INFO.'
          structOK = 0
      endif

      ind = WHERE(tagNames eq 'LATD', count)
      if (count eq 0) then begin
          ERR_MSG, 'No "latD" tag in RTMA197_GRID_PROJ_INFO.'
          structOK = 0
      endif

      ind = WHERE(tagNames eq 'LATSEC1', count)
      if (count eq 0) then begin
          ERR_MSG, 'No "latSec1" tag in RTMA197_GRID_PROJ_INFO.'
          structOK = 0
      endif

      ind = WHERE(tagNames eq 'LATSEC2', count)
      if (count eq 0) then begin
          ERR_MSG, 'No "latSec2" tag in RTMA197_GRID_PROJ_INFO.'
          structOK = 0
      endif

      ind = WHERE(tagNames eq 'ERADM', count)
      if (count eq 0) then begin
          ERR_MSG, 'No "eRadM" tag in RTMA197_GRID_PROJ_INFO.'
          structOK = 0
      endif

      ind = WHERE(tagNames eq 'LAT00', count)
      if (count eq 0) then begin
          ERR_MSG, 'No "lat00" tag in RTMA197_GRID_PROJ_INFO.'
          structOK = 0
      endif

      ind = WHERE(tagNames eq 'LON00', count)
      if (count eq 0) then begin
          ERR_MSG, 'No "lon00" tag in RTMA197_GRID_PROJ_INFO.'
          structOK = 0
      endif

      ind = WHERE(tagNames eq 'NCOLS', count)
      if (count eq 0) then begin
          ERR_MSG, 'No "nCols" tag in RTMA197_GRID_PROJ_INFO.'
          structOK = 0
      endif

      ind = WHERE(tagNames eq 'NROWS', count)
      if (count eq 0) then begin
          ERR_MSG, 'No "nRows" tag in RTMA197_GRID_PROJ_INFO.'
          structOK = 0
      endif

      ind = WHERE(tagNames eq 'DX', count)
      if (count eq 0) then begin
          ERR_MSG, 'No "dx" tag in RTMA197_GRID_PROJ_INFO.'
          structOK = 0
      endif

      ind = WHERE(tagNames eq 'DY', count)
      if (count eq 0) then begin
          ERR_MSG, 'No "dy" tag in RTMA197_GRID_PROJ_INFO.'
          structOK = 0
      endif

      if structOK then begin

          if NOT(ISA(RTMA197GridProjInfo.lonV, 'DOUBLE')) then begin
              ERR_MSG, 'RTMA197_GRID_PROJ_INFO missing DOUBLE element ' + $
                       '"lonV".'
              structOK = 0
          endif

          if NOT(ISA(RTMA197GridProjInfo.latD, 'DOUBLE')) then begin
              ERR_MSG, 'RTMA197_GRID_PROJ_INFO missing DOUBLE element ' + $
                       '"latD".'
              structOK = 0
          endif

          if NOT(ISA(RTMA197GridProjInfo.latSec1, 'DOUBLE')) then begin
              ERR_MSG, 'RTMA197_GRID_PROJ_INFO missing DOUBLE element ' + $
                       '"latSec1".'
              structOK = 0
          endif

          if NOT(ISA(RTMA197GridProjInfo.latSec2, 'DOUBLE')) then begin
              ERR_MSG, 'RTMA197_GRID_PROJ_INFO missing DOUBLE element ' + $
                       '"latSec2".'
              structOK = 0
          endif

          if NOT(ISA(RTMA197GridProjInfo.eRadM, 'DOUBLE')) then begin
              ERR_MSG, 'RTMA197_GRID_PROJ_INFO missing DOUBLE element ' + $
                       '"eRadM".'
              structOK = 0
          endif

          if NOT(ISA(RTMA197GridProjInfo.lat00, 'DOUBLE')) then begin
              ERR_MSG, 'RTMA197_GRID_PROJ_INFO missing DOUBLE element ' + $
                       '"lat00".'
              structOK = 0
          endif

          if NOT(ISA(RTMA197GridProjInfo.lon00, 'DOUBLE')) then begin
              ERR_MSG, 'RTMA197_GRID_PROJ_INFO missing DOUBLE element ' + $
                       '"lon00".'
              structOK = 0
          endif

          if NOT(ISA(RTMA197GridProjInfo.nCols, 'LONG')) then begin
              ERR_MSG, 'RTMA197_GRID_PROJ_INFO missing LONG element ' + $
                       '"nCols".'
              structOK = 0
          endif

          if NOT(ISA(RTMA197GridProjInfo.nRows, 'LONG')) then begin
              ERR_MSG, 'RTMA197_GRID_PROJ_INFO missing LONG element ' + $
                       '"nRows".'
              structOK = 0
          endif

          if NOT(ISA(RTMA197GridProjInfo.dx, 'DOUBLE')) then begin
              ERR_MSG, 'RTMA197_GRID_PROJ_INFO missing DOUBLE element ' + $
                       '"dx".'
              structOK = 0
          endif

          if NOT(ISA(RTMA197GridProjInfo.dy, 'DOUBLE')) then begin
              ERR_MSG, 'RTMA197_GRID_PROJ_INFO missing DOUBLE element ' + $
                       '"dy".'
              structOK = 0
          endif
      ...
      if NOT(structOK) then begin
          ...
          ERR_MSG, 'Unexpected RTMA197_GRID_PROJ_INFO structure definition.'
          RETURN
  ...
  savFile = 'RTMA_197_MIN_MAX_AVE_TMP' + $
...
;-
      ...
      if NOT(ISA(RTMA197GridProjInfo_)) then begin
          ERR_MSG, 'No RTMA 197 grid/projection info structure in ' + $
                   ScratchDir + '/' + savFile
          minTmpGrid = !NULL
          maxTmpGrid = !NULL
          aveTmpGrid = !NULL
...
;+
;     Verify the structure of RTMA197GridProjInfo_ and grid dimensions.
;-
      foo = SIZE(RTMA197GridProjInfo_)
      if (foo[0] ne 1) then begin
          ERR_MSG, 'RTMA197GridProjInfo_ structure mismatch (non-scalar) ' + $
      ...
      if ((foo[1] ne 1) or (foo[2] ne 8)) then begin
          ERR_MSG, 'RTMA197GridProjInfo_ structure mismatch ' + $
                   '(not a structure) ' + $
                   'in ' + ScratchDir + '/' + savFile
          minTmpGrid = !NULL
          maxTmpGrid = !NULL
      ...
      tagNames = TAG_NAMES(RTMA197GridProjInfo_)
      ind = WHERE(tagNames eq 'LONV', count)
      if (count eq 0) then begin
          ERR_MSG, 'No "lonV" tag in "RTMA197GridProjInfo_" from ' + $
      ...
      ind = WHERE(tagNames eq 'LATD', count)
      if (count eq 0) then begin
          ERR_MSG, 'No "latD" tag in "RTMA197GridProjInfo_" from ' + $
                   'IDL save file ' + ScratchDir + '/' + savFile
          savFileOK = 0
      endif
      ...
      ind = WHERE(tagNames eq 'LATSEC1', count)
      if (count eq 0) then begin
          ERR_MSG, 'No "latSec1" tag in "RTMA197GridProjInfo_" from ' + $
                   'IDL save file ' + ScratchDir + '/' + savFile
          savFileOK = 0
      endif
      ...
      ind = WHERE(tagNames eq 'LATSEC2', count)
      if (count eq 0) then begin
          ERR_MSG, 'No "latSec2" tag in "RTMA197GridProjInfo_" from ' + $
                   'IDL save file ' + ScratchDir + '/' + savFile
          savFileOK = 0
      endif
      ...
      ind = WHERE(tagNames eq 'ERADM', count)
      if (count eq 0) then begin
          ERR_MSG, 'No "eRadM" tag in "RTMA197GridProjInfo_" from ' + $
                   'IDL save file ' + ScratchDir + '/' + savFile
          savFileOK = 0
      endif
      ...
      ind = WHERE(tagNames eq 'LAT00', count)
      if (count eq 0) then begin
          ERR_MSG, 'No "lat00" tag in "RTMA197GridProjInfo_" from ' + $
                   'IDL save file ' + ScratchDir + '/' + savFile
          savFileOK = 0
      endif
      ...
      ind = WHERE(tagNames eq 'LON00', count)
      if (count eq 0) then begin
          ERR_MSG, 'No "lon00" tag in "RTMA197GridProjInfo_" from ' + $
                   'IDL save file ' + ScratchDir + '/' + savFile
          savFileOK = 0
      endif
      ...
      ind = WHERE(tagNames eq 'NCOLS', count)
      if (count eq 0) then begin
          ERR_MSG, 'No "nCols" tag in "RTMA197GridProjInfo_" from ' + $
                   'IDL save file ' + ScratchDir + '/' + savFile
          savFileOK = 0
      endif
      ...
      ind = WHERE(tagNames eq 'NROWS', count)
      if (count eq 0) then begin
          ERR_MSG, 'No "nRows" tag in "RTMA197GridProjInfo_" from ' + $
                   'IDL save file ' + ScratchDir + '/' + savFile
          savFileOK = 0
      endif
      ...
      ind = WHERE(tagNames eq 'DX', count)
      if (count eq 0) then begin
          ERR_MSG, 'No "dx" tag in "RTMA197GridProjInfo_" from ' + $
                   'IDL save file ' + ScratchDir + '/' + savFile
          savFileOK = 0
      endif
      ...
      ind = WHERE(tagNames eq 'DY', count)
      if (count eq 0) then begin
          ERR_MSG, 'No "dy" tag in "RTMA197GridProjInfo_" from ' + $
                   'IDL save file ' + ScratchDir + '/' + savFile
          savFileOK = 0
      endif
      ...
      if savFileOK then begin
          ...
          if NOT(ISA(RTMA197GridProjInfo_.lonV, 'DOUBLE')) then begin
              ERR_MSG, '"RTMA197GridProjInfo_" missing DOUBLE element ' + $
                       '"lonV" ' + $
                       'in ' + ScratchDir + '/' + savFile
              savFileOK = 0
          endif

          if NOT(ISA(RTMA197GridProjInfo_.latD, 'DOUBLE')) then begin
              ERR_MSG, '"RTMA197GridProjInfo_" missing DOUBLE element ' + $
                       '"latD" ' + $
                       'in ' + ScratchDir + '/' + savFile
              savFileOK = 0
          ...
          if NOT(ISA(RTMA197GridProjInfo_.latSec1, 'DOUBLE')) then begin
              ERR_MSG, '"RTMA197GridProjInfo_" missing DOUBLE element ' + $
                       '"latSec1" ' + $
                       'in ' + ScratchDir + '/' + savFile
              savFileOK = 0
          ...
          if NOT(ISA(RTMA197GridProjInfo_.latSec2, 'DOUBLE')) then begin
              ERR_MSG, '"RTMA197GridProjInfo_" missing DOUBLE element ' + $
                       '"latSec2" ' + $
                       'in ' + ScratchDir + '/' + savFile
              savFileOK = 0
          ...
          if NOT(ISA(RTMA197GridProjInfo_.eRadM, 'DOUBLE')) then begin
              ERR_MSG, '"RTMA197GridProjInfo_" missing DOUBLE element ' + $
                       '"eRadM" ' + $
                       'in ' + ScratchDir + '/' + savFile
              savFileOK = 0
          ...
          if NOT(ISA(RTMA197GridProjInfo_.lat00, 'DOUBLE')) then begin
              ERR_MSG, '"RTMA197GridProjInfo_" missing DOUBLE element ' + $
                       '"lat00" ' + $
                       'in ' + ScratchDir + '/' + savFile
              savFileOK = 0
          ...
          if NOT(ISA(RTMA197GridProjInfo_.lon00, 'DOUBLE')) then begin
              ERR_MSG, '"RTMA197GridProjInfo_" missing DOUBLE element ' + $
                       '"lon00" ' + $
                       'in ' + ScratchDir + '/' + savFile
              savFileOK = 0
          ...
          if NOT(ISA(RTMA197GridProjInfo_.nCols, 'LONG')) then begin
              ERR_MSG, '"RTMA197GridProjInfo_" missing LONG element ' + $
                       '"nCols" ' + $
                       'in ' + ScratchDir + '/' + savFile
              savFileOK = 0
          endif else begin
              if (nCols ne RTMA197GridProjInfo_.nCols) then begin
                  ERR_MSG, 'Grid columns in ' + $
                           ScratchDir + '/' + savFile + $
                           ' (' + STRCRA(nCols) + ') ' + $
                           'do not match "RTMA197GridProjInfo_" value ' + $
                           '(' + STRCRA(RTMA197GridProjInfo_.nCols) + ').'
                  savFileOK = 0
              endif
          endelse

          if NOT(ISA(RTMA197GridProjInfo_.nRows, 'LONG')) then begin
              ERR_MSG, '"RTMA197GridProjInfo_" missing LONG element ' + $
                       '"nRows" ' + $
                       'in ' + ScratchDir + '/' + savFile
              savFileOK = 0
          endif else begin
              if (nRows ne RTMA197GridProjInfo_.nRows) then begin
                  ERR_MSG, 'Grid rows in ' + $
                           ScratchDir + '/' + savFile + $
                           ' (' + STRCRA(nRows) + ') ' + $
                           'do not match "RTMA197GridProjInfo_" value ' + $
                           '(' + STRCRA(RTMA197GridProjInfo_.nRows) + ').'
                  savFileOK = 0
              endif
          endelse

          if NOT(ISA(RTMA197GridProjInfo_.dx, 'DOUBLE')) then begin
              ERR_MSG, '"RTMA197GridProjInfo_" missing DOUBLE element ' + $
                       '"dx" ' + $
                       'in ' + ScratchDir + '/' + savFile
              savFileOK = 0
          ...
          if NOT(ISA(RTMA197GridProjInfo_.dy, 'DOUBLE')) then begin
              ERR_MSG, '"RTMA197GridProjInfo_" missing DOUBLE element ' + $
                       '"dy" ' + $
                       'in ' + ScratchDir + '/' + savFile
              savFileOK = 0
      ...
      if (savFileOK and KEYWORD_SET(RTMA197GridProjInfo)) then begin

;+
;         Verify RTMA197GridProjInfo structure from caller against
;         RTMA197GridProjInfo_ structure from save file.
;-
          if NOT(COMPARE(RTMA197GridProjInfo.lonV, $
                         RTMA197GridProjInfo_.lonV)) $
              then begin
              ERR_MSG, 'RTMA197_GRID_PROJ_INFO structure "lonV" mismatch ' + $
                       'between keyword and ' + $
                       ScratchDir + '/' + savFile + ' data.'
              savFileOK = 0
          endif

          if NOT(COMPARE(RTMA197GridProjInfo.latD, $
                         RTMA197GridProjInfo_.latD)) $
              then begin
              ERR_MSG, 'RTMA197_GRID_PROJ_INFO structure "latD" mismatch ' + $
                       'between keyword and ' + $
                       ScratchDir + '/' + savFile + ' data.'
              savFileOK = 0
          endif

          if NOT(COMPARE(RTMA197GridProjInfo.latSec1, $
                         RTMA197GridProjInfo_.latSec1)) $
              then begin
              ERR_MSG, 'RTMA197_GRID_PROJ_INFO structure "latSec1" ' + $
                       'mismatch between keyword and ' + $
                       ScratchDir + '/' + savFile + ' data.'
              savFileOK = 0
          endif

          if NOT(COMPARE(RTMA197GridProjInfo.latSec2, $
                         RTMA197GridProjInfo_.latSec2)) $
              then begin
              ERR_MSG, 'RTMA197_GRID_PROJ_INFO structure "latSec2" ' + $
                       'mismatch between keyword and ' + $
                       ScratchDir + '/' + savFile + ' data.'
              savFileOK = 0
          endif

          if NOT(COMPARE(RTMA197GridProjInfo.eRadM, $
                         RTMA197GridProjInfo_.eRadM)) $
              then begin
              ERR_MSG, 'RTMA197_GRID_PROJ_INFO structure "eRadM" ' + $
                       'mismatch between keyword and ' + $
                       ScratchDir + '/' + savFile + ' data.'
              savFileOK = 0
          endif

          if NOT(COMPARE(RTMA197GridProjInfo.lat00, $
                         RTMA197GridProjInfo_.lat00)) $
              then begin
              ERR_MSG, 'RTMA197_GRID_PROJ_INFO structure "lat00" mismatch ' + $
                       'between keyword and ' + $
                       ScratchDir + '/' + savFile + ' data.'
              savFileOK = 0
          endif

          if NOT(COMPARE(RTMA197GridProjInfo.lon00, $
                         RTMA197GridProjInfo_.lon00)) $
              then begin
              ERR_MSG, 'RTMA197_GRID_PROJ_INFO structure "lon00" mismatch ' + $
                       'between keyword and ' + $
                       ScratchDir + '/' + savFile + ' data.'
              savFileOK = 0
          endif

          if (RTMA197GridProjInfo.nCols ne $
              RTMA197GridProjInfo_.nCols) then begin
              ERR_MSG, 'RTMA197_GRID_PROJ_INFO structure "nCols" mismatch ' + $
                       'between keyword and ' + $
                       ScratchDir + '/' + savFile + ' data.'
              savFileOK = 0
          endif

          if (RTMA197GridProjInfo.nRows ne $
              RTMA197GridProjInfo_.nRows) then begin
              ERR_MSG, 'RTMA197_GRID_PROJ_INFO structure "nRows" mismatch ' + $
                       'between keyword and ' + $
                       ScratchDir + '/' + savFile + ' data.'
              savFileOK = 0
          endif

          if NOT(COMPARE(RTMA197GridProjInfo.dx, RTMA197GridProjInfo_.dx)) $
              then begin
              ERR_MSG, 'RTMA197_GRID_PROJ_INFO structure "dx" mismatch ' + $
                       'between keyword and ' + $
                       ScratchDir + '/' + savFile + ' data.'
              savFileOK = 0
          endif

          if NOT(COMPARE(RTMA197GridProjInfo.dy, RTMA197GridProjInfo_.dy)) $
              then begin
              ERR_MSG, 'RTMA197_GRID_PROJ_INFO structure "dy" mismatch ' + $
                       'between keyword and ' + $
                       ScratchDir + '/' + savFile + ' data.'
              savFileOK = 0
          endif
      ...
      if (savFileOK and NOT(KEYWORD_SET(RTMA197GridProjInfo))) then begin

;+
;         Copy RTMA197GridProjInfo structure from save file.
;-
          RTMA197GridProjInfo = RTMA197GridProjInfo_
...
;-
  ...
  for hc = 0, DurationHours - 1 do begin
      ...
      GET_HOURLY_RTMA197_2M_TEMP, $
          hourStartDate_YYYYMMDDHH, $
          RTMADir, $
          ScratchDir, $
          NoDataValue, $
          hourlyTmpGrid, $
          RTMA197_GRID_PROJ_INFO = RTMA197GridProjInfo, $
          VERBOSE = KEYWORD_SET(verbose)

      if NOT(ISA(hourlyTmpGrid)) then begin
          if KEYWORD_SET(verbose) then $
              ERR_MSG, 'Failed to get RTMA grid 197 ' + $
...
;ind = where(mintmpgrid_ ne nodatavalue, count)
;print, count
;if (count gt 0) then print, 'minimum range: ', min(mintmpgrid_[ind]), max(mintmpgrid_[ind])
...
;-
      RTMA197GridProjInfo_ = RTMA197GridProjInfo
      ndv_ = NoDataValue

      SAVE, minTmpGrid, maxTmpGrid, aveTmpGrid, $
            RTMA197GridProjInfo_, ndv_, $
