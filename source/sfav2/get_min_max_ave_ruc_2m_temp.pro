PRO GET_MIN_MAX_AVE_RUC_2M_TEMP, $
...
;+
; Get hourly Rapid Update Cycle (RUC) 2-meter air temperatures (TMP)
; from GRIB files, determine gridded minimum, maximum, and mean
...
;         The minimum RUC "2 m above ground" temperature across the
;         analysis period.
;     maxTmpGrid : out, type=FLTARR(nx x ny)
;         The maximum RUC "2 m above ground" temperature across the
;         analysis period.
;     aveTmpGrid : out, type=FLTARR(nx x ny)
;         The average RUC "2 m above ground" temperature across the
;         analysis period.
...
; Comment out this section for debugging.
;-
  CATCH, errorStatus
...
;-
  ...
  savFile = 'RUC_LONLAT_MIN_MAX_AVE_TMP' + $
...
;-
  ...
  GET_MIN_MAX_AVE_RUC130_2M_TEMP, $
      AnalysisEndDate_YYYYMMDDHH, $
      DurationHours, $
      TargetFcstHour, $
      MinSubFcstHour, $
  ...
  if (NOT(ISA(minTmpGrid130)) or $
      NOT(ISA(maxTmpGrid130)) or $
      NOT(ISA(aveTmpGrid130))) then begin
      ERR_MSG, 'Failed to get min/max/ave RUC temperatures for ' + $
...
; Perform regridding.
;-
  minTmpGrid_ = REGRID_BILIN(minTmpGrid130, $
                             iGrid_RUC, $
                             jGrid_RUC, $
                             NoDataValue)
  if NOT(ISA(minTmpGrid_)) then RETURN
  maxTmpGrid_ = REGRID_BILIN(maxTmpGrid130, $
                             iGrid_RUC, $
                             jGrid_RUC, $
                             NoDataValue)
  if NOT(ISA(maxTmpGrid_)) then RETURN
  aveTmpGrid_ = REGRID_BILIN(aveTmpGrid130, $
                             iGrid_RUC, $
                             jGrid_RUC, $
                             NoDataValue)
