PRO GET_MIN_MAX_AVE_RTMA5_2M_TEMP, $
...
; temperatures (TMP) from grid 197 (the 5 km CONUS grid) GRIB files,
; determine gridded minimum, maximum, and mean values on the grid, and
; regrid/deproject the data to longitude/latitude coordinates with the
; designated geographic domain and resolution. The "5" in the name
; of this procedure refers not to the resolution of the output data,
; but rather (roughly) to the native resolution of the input data
; used, to distinguish it from 2.5 km RTMA data that has been
; available since September 2010. A separate set of procedures must be
; used to read those.
;
; :Params:
;
...
;         against the contents of all the input GRIB2 files. Note that
;         the dimensions of grid 197, defined by this structure, will
;         not match those of the output grids, which are on a
...
; "if (NOT(ISA(accumACPCPGrid)) or NOT(ISA(maxTmpGrid)) or
...
;-
  ...
  savFile = 'RTMA5_LONLAT_MIN_MAX_AVE_TMP' + $
...
;+
; Read RTMA grid 197 data from GRIB file/s and regrid. This is really
; the beginning of the process if we do not have a SAVE file.
;-
  minTmpGrid197 = !NULL
  maxTmpGrid197 = !NULL
  aveTmpGrid197 = !NULL

  GET_MIN_MAX_AVE_RTMA197_2M_TEMP, $
      AnalysisEndDate_YYYYMMDDHH, $
      DurationHours, $
      MaxMissingHours, $
      RTMADir, $
  ...
  if (NOT(ISA(minTmpGrid197)) or $
      NOT(ISA(maxTmpGrid197)) or $
      NOT(ISA(aveTmpGrid197))) then begin
      ERR_MSG, 'Failed to get min/max/ave RTMA (grid 197) temperatures ' + $
               'for ' + STRCRA(DurationHours) + ' hours ending ' + $
               AnalysisEndDate_YYYYMMDDHH
      RETURN
  endif
...
;-
  ...
  LCC_GRIB2_TO_SNYDER, RTMA197GridProjInfo.latSec1, $
                       RTMA197GridProjInfo.latSec2, $
                       RTMA197GridProjInfo.latD, $
                       RTMA197GridProjInfo.lonV, $
                       RTMA197GridProjInfo.lat00, $
                       RTMA197GridProjInfo.lon00, $
                       RTMA197GridProjInfo.eRadM, $
                       lonV_rad, $
                       nSny, $
                       FSny, $
                       rho0, $
  ...
  degToRad = !DPi / 180.0D
  rho = RTMA197GridProjInfo.eRadM * FSny / $
        (TAN(!DPi / 4.0D + latGrid * degToRad / 2.0D))^nSny ; Snyder 15-1
  theta_rad = nSny * (lonGrid * degToRad - lonV_rad)
  x = rho * SIN(theta_rad)        ; x location on RTMA (197) grid
  y = rho0 - rho * COS(theta_rad) ; y location on RTMA (197) grid

  rho = !NULL
  theta_rad = !NULL

  iGrid_RTMA197 = (x - x00) / RTMA197GridProjInfo.dx
  jGrid_RTMA197 = (y - y00) / RTMA197GridProjInfo.dy
...
; Perform regridding.
;-
  minTmpGrid_ = REGRID_BILIN(minTmpGrid197, $
                             iGrid_RTMA197, $
                             jGrid_RTMA197, $
                             NoDataValue)
  if NOT(ISA(minTmpGrid_)) then RETURN
  maxTmpGrid_ = REGRID_BILIN(maxTmpGrid197, $
                             iGrid_RTMA197, $
                             jGrid_RTMA197, $
                             NoDataValue)
  if NOT(ISA(maxTmpGrid_)) then RETURN
  aveTmpGrid_ = REGRID_BILIN(aveTmpGrid197, $
                             iGrid_RTMA197, $
                             jGrid_RTMA197, $
                             NoDataValue)
