PRO GET_ACCUM_HRRR_LC_APCP_WEASD, $
...
;+
; Get accumulated hourly High Resolution Rapid Refresh (HRRR)
; quantitative precipitation forecasts (QPF), including total
; precipitation (APCP) and water equivalent of accumulated snow depth
; (WEASD) from GRIB files, on the 3 km CONUS Lambert conformal grid
; for HRRR.
;
...
;     accumAPCPGrid : out, type=FLTARR(1799 x 1059)
;         The aggregated HRRR "total precipitation".
;     accumWEASDGrid : out, type=FLTARR(1799 x 1059)
;         The aggregated HRRR "water equivalent of accumulated snow
;         depth".
;     perfect : out, type=BYTE
;         A flag that indicates whether (1) or not (0) data was found
;         for the TargetFcstHour for every hour of the accumulation.
;
; :Keywords:
;
...
; RESTORE encounters a file that was truncated because a disk
; filled. Comment this section out for debugging.
;-
  ;; CATCH, errorStatus
...
;-
  ...
  for hc = 0, DurationHours - 1 do begin
      ...
      if (NOT(ISA(hourlyAPCPGrid)) or $
          ...
          if KEYWORD_SET(Verbose) then $
              ERR_MSG, 'Failed to get HRRR precipitation for hour ending ' + $
                       hourEndDate_YYYYMMDDHH
          RETURN
      endif
