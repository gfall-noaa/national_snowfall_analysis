PRO READ_GHCND_US_CA_MX_BY_YEAR, GHCND_dir, $
                                 Year, $
...
; % grep "^US\|^CA\|^MX" 2014.csv | \
;   grep "PRCP\|SNOW" > GHCN_US_CA_MX_PRCP_SNOW_2014.csv
;
...
;   [prcp,snow]_mflag - measurement flag for observation (see below)
;   [prcp,snow]_qflag - quality flag for observation (see below)
;   [prcp,snow]_sflag - source flag for observation (see below)
;   [prcp,wnow]_time_HHMM - local time of observation
;
...
;-
  ...
  GHCNFile = 'GHCN_US_CA_MX_PRCP_SNOW_' + YYYY + '.csv'
  saveFile = 'GHCN_US_CA_MX_PRCP_SNOW_' + YYYY + '.sav'
...
;-
      ...
      if (N_ELEMENTS(tagNames) lt 10) then STOP
      if (N_ELEMENTS(tagNames) gt 10) then $
          MESSAGE, 'WARNING: GHCNData structure in ' + $
                   GHCND_dir + '/by_year/' + saveFile + $
                   ' has >10 tags.', /CONTINUE
...
;-
  ...
  while NOT(EOF(lun)) do begin
      ...
      ;; old code that collected other GHCND fields
      ;; 'SNWD': begin
      ;;     GHCNData[dayInd, idInd].snwd = LONG(value)
      ;;     GHCNData[dayInd, idInd].snwd_mflag = mFlag
      ;;     GHCNData[dayInd, idInd].snwd_qflag = mFlag
      ;;     GHCNData[dayInd, idInd].snwd_sflag = mFlag
      ;;     GHCNData[dayInd, idInd].snwd_time_HHMM = time_HHMM
      ;; end
      ;; 'TMAX': begin
      ;;     GHCNData[dayInd, idInd].tmax = LONG(value)
      ;;     GHCNData[dayInd, idInd].tmax_mflag = mFlag
      ;;     GHCNData[dayInd, idInd].tmax_qflag = mFlag
      ;;     GHCNData[dayInd, idInd].tmax_sflag = mFlag
      ...
      ;;     GHCNData[dayInd, idInd].tmin_qflag = mFlag
      ;;     GHCNData[dayInd, idInd].tmin_sflag = mFlag
      ;;     GHCNData[dayInd, idInd].tmin_time_HHMM = time_HHMM
      ;; end
      ;; 'TAVE': begin
      ;;     GHCNData[dayInd, idInd].tave = LONG(value)
      ;;     GHCNData[dayInd, idInd].tave_mflag = mFlag
      ;;     GHCNData[dayInd, idInd].tave_qflag = mFlag
      ;;     GHCNData[dayInd, idInd].tave_sflag = mFlag
      ;;     GHCNData[dayInd, idInd].tave_time_HHMM = time_HHMM
      ;; end
...
;-
      ...
      for dc = 0, numDays - 1 do begin
          ...
          bothInd = WHERE((dailyData.prcp ne Ndv) and $
                          (dailyData.snow ne Ndv), $
                          bothCount)
          prcpInd = WHERE((dailyData.prcp ne Ndv) and $
                          (dailyData.snow eq Ndv), $
                          prcpCount)
          snowInd = WHERE((dailyData.prcp eq Ndv) and $
                          (dailyData.snow ne Ndv), $
                          snowCount)
          PRINT, ' ' + date_YYYYMMDD + ' | ' + $
